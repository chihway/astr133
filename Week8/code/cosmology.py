...
def dcom(z, Om0, OmL, ninter=20):
    ...
    if nz == 1:
        if np.abs(a-1.0) < 1.e-10:
            ...
            dc = romberg(d_func, a, 1., rtol = 1.e-10, mmax = 8, verbose = False, **kwargs)[0]
    elif nz > 1:
        dc = np.zeros(nz)
        if nz <= ninter:
            for i, ad in enumerate(a):
                if np.abs(ad-1.0) < 1.e-10:
                    ...
                    dc[i] = romberg(d_func, ad, 1., rtol = 1.e-10, mmax = 8, verbose = False, **kwargs)[0]
        else:
            ...
            for i, zid in enumerate(zi):
                ...
                if np.abs(aid-1.0) < 1.e-10:
                    ...
                    fi[i] = romberg(d_func, aid, 1., rtol = 1.e-10, mmax = 8, verbose = False, **kwargs)[0]
